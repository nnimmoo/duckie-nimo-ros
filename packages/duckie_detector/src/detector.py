...
from duckie_detector import duckiedetector, ColorRange, plotSegments, plotMaps
...
class duckiedetectorNode(DTROS):
    """
    The ``duckiedetector
Node`` is responsible for detecting the line white, yellow and red line segment in an image and
    ...
    road-containing part of the image is and yellow segments and publishes them.
    The main functionality of this node is implemented in the :py:class:`duckie_detector.duckiedetector
` class.
    Args:
        node_name (:obj:`str`): a unique, descriptive name for the node that ROS will use

    Configuration:
        ~duckie_detector_parameters (:obj:`dict`): A dictionary with the parameters for the detector. The full list can be found in :py:class:`duckie_detector.duckiedetector
    `.
    ...
    def __init__(self, node_name):
        ...
        super(duckiedetectorNode, self).__init__(node_name=node_name, node_type=NodeType.PERCEPTION)

        # Define parameters
        self._duckie_detector_parameters = rospy.get_param("~duckie_detector_parameters", None)
        ...
        # Create a new duckiedetector
        # object with the parameters from the Parameter Server / config file
        self.detector = duckiedetector(**self._duckie_detector_parameters)
    ...
    def image_cb(self, image_msg):
        ...
        #. Extracts the line segments in the image using :py:class:`duckie_detector.duckiedetector
        #`
...
if __name__ == "__main__":
    # Initialize the node
    duckie_detector_node = duckiedetectorNode(node_name="duckie_detector_node")
